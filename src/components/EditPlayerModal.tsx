...
export function EditPlayerModal({ isOpen, onClose, player, onPlayerUpdated }: EditPlayerModalProps) {
  ...
  // Load teams and populate form when modal opens
  useEffect(() => {
    if (isOpen && player) {
      loadTeams();
      populateForm();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!player?.id) {
      toast.error('O\'yinchi topilmadi');
      return;
    }
    ...
    try {
      ...
      let photoBase64 = formData.photoPreview;
      if (formData.photo) {
      ...
      toast.error('O\'yinchi ma\'lumotlarini yangilashda xatolik');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="O'yinchi Ma'lumotlarini Tahrirlash">
