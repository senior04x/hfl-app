// Phone number formatting utilities for mobile app
...
export const parsePhoneNumberForAPI = (value: string): string => {
  ...
  // If it already starts with 998, return as is
  if (digits.startsWith('998')) {
    return `+${digits}`;
  }
  
  // If it's 9 digits starting with 9, add 998 prefix
  if (digits.length === 9 && digits.startsWith('9')) {
    return `+998${digits}`;
  }
  
  // Default: add 998 prefix
  return `+998${digits}`;
};
